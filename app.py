...
def insert_lineups(df: pd.DataFrame) -> None:
    ...
    for _, r in df.iterrows():
        ...
        if team_val not in ("Non-bibs","Bibs"):
            # try case-insensitive
            if team_val.lower() == "non-bibs":
                team_val = "Non-bibs"
            elif team_val.lower() == "bibs":
                team_val = "Bibs"
    ...
    keyset = {(r["match_id"], r["team"]) for r in rows if r["match_id"] and r["team"]}
    for (mid, team) in sorted(keyset):
