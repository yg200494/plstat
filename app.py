...
def upsert_matches(df: pd.DataFrame) -> None:
    ...
    # If is_draw missing, infer from scores
    if "is_draw" in df.columns:
        df["is_draw"] = df.apply(lambda r: bool(r.get("is_draw")) if pd.notna(r.get("is_draw")) else (int(r.get("score_a") or 0) == int(r.get("score_b") or 0)), axis=1)
...
def insert_lineups(df: pd.DataFrame) -> None:
    # PATCH: resolve match_id from season+gw if missing

    sb = get_service_client()
    if sb is None:
        st.error("Admin required.")
        return
    # map player_name to id if provided
    players = fetch_players()
    name_to_id = {n.strip().lower(): pid for n,pid in zip(players["name"].fillna(""), players["id"].fillna(""))}
    rows = []
    for _, r in df.iterrows():
        # Resolve match_id if blank using season+gw
        mid = r.get('match_id')
        ...
        rid = {
            "season": int(r.get("season")) if not pd.isna(r.get("season")) else None,
            "gw": int(r.get("gw")) if not pd.isna(r.get("gw")) else None,
            "match_id": mid,
            "team": r.get("team"),
            "player_name": r.get("player_name"),
            "player_id": r.get("player_id") or name_to_id.get(str(r.get("player_name") or "").strip().lower()),
            "is_gk": bool(int(r.get("is_gk"))) if str(r.get("is_gk")).strip() not in ["", "None", "nan"] else False,
    ...
    for (mid, team) in sorted({(r["match_id"], r["team"]) for r in rows}):
        sb.table("lineups").delete().eq("match_id", mid).eq("team", team).execute()
        to_ins = [r for r in rows if r["match_id"] == mid and r["team"] == team]
        if to_ins:
            sb.table("lineups").insert(to_ins).execute()
    clear_caches()
...
pages = {
    ...
    "Stats": page_stats,
    "Awards": page_awards,
    "Import/Export": page_import_export if st.session_state.get("is_admin") else None
}
# Filter out None pages to avoid navigation errors
